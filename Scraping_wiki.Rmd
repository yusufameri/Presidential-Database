...
writeFD <- function(df) {
    for(i in 1:nrow(df)) {
      ...
      sample <-         df$sample
      moe <-            df$moe
      questiontxt <-    df$questiontxt
      surveyorg <-      df$surveyorg
      surveysponsor <-  df$surveysponsor
      intmethod <-      df$intmethod
      sampledesc <-     df$sampledesc
      state <-          df$state
      inttype <-        df$pollType
...
### We should replace all of those NAs with NULL for SQL purposes
```{r}
